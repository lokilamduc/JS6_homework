...
function showInputStudent(action, student) {
  ...
  $("#inputList").html(`
        <label>ID</label>
        <input id='inputID'class='w-100' value="${
        ...
        <label>Name</label>
        <input id='inputName' class='w-100' value="${
          action === "Update" ? student.name : ""
        }"/>
        <div id='nameError' class='text-danger d-none'>Input Name again, only containing letters and spaces</div>
        <label>Address</label>
        <input id='inputAddress' class='w-100' value="${
          action === "Update" ? student.address : ""
        }"/>
        <div id='addressError' class='text-danger d-none'>Input address again</div>
        <label>Email</label>
        <input id='inputEmail' class='w-100' value="${
          action === "Update" ? student.email : ""
        }"/>
        <div id='emailError' class='text-danger d-none'>Input email again</div>
        <label>Math</label>
        <input id='inputMath' class='w-100' value="${
          action === "Update" ? student.math : ""
        }"/>
        <div id='mathError' class='text-danger d-none'>Input mark again, from 0 to 10</div>
        <label>Physics</label>
        <input id='inputPhysics' class='w-100' value="${
          action === "Update" ? student.physics : ""
        }"/>
        <div id='physicsError' class='text-danger d-none'>Input mark again, from 0 to 10</div>
        <label>Chemistry</label>
        <input id='inputChemistry' class='w-100' value="${
          action === "Update" ? student.chemistry : ""
        }"/>
...
function showInputEmployee(action, employee) {
  ...
  $("#inputList").html(`
        ...
        <input id='inputDays' class='w-100' value="${
          action === "Update" ? employee.days : ""
        }"/>
        <div id='daysError' class='text-danger d-none'>Input working days again</div>
        <label>Salary per day</label>
        <input id='inputSalaryUnit' class='w-100' value="${
          action === "Update" ? employee.salaryUnit : ""
        }"/>
...
function showInputCustomer(action, customer) {
  ...
  $("#inputList").html(`
        ...
        <input id='inputNameCompany' class='w-100' value="${
          action === "Update" ? customer.nameCompany : ""
        }"/>
        <div id='nameCompanyError' class='text-danger d-none'>Input name of company again</div>
        <label>Bill</label>
        <input id='inputBill' class='w-100' value="${
          action === "Update" ? customer.bill : ""
        }"/>
        ...
        <select id='inputRate' class='w-50'>
            ...
            <option value='1' ${
              action === "Update" && customer.rate === "Good" ? "selected" : ""
            }>Good</option>
            <option value='2' ${
              action === "Update" && customer.rate === "Bad" ? "selected" : ""
            }>Bad</option>
        </select>
    `);
}
...
function validateGeneralInformation(person) {
  ...
  //check name
  if (!/[a-z]|[A-Z]|\s\D/g.test(person.name)) {
...
function validateStudent(student) {
  ...
  if (!validateGeneralInformation(student)) isValidated = false;

  //check marks
  if (
    !/^\d+$/.test(student.math) ||
    (student.math.length > 1 && +student.math > 10)
  ) {
  ...
  if (
    !/^\d+$/.test(student.physics) ||
    (student.physics.length > 1 && +student.physics > 10)
  ) {
  ...
  if (
    !/^\d+$/.test(student.chemistry) ||
    (student.chemistry.length > 1 && +student.chemistry > 10)
  ) {
...
function validateEmployee(employee) {
  ...
  if (!validateGeneralInformation(employee)) isValidated = false;
...
function validateCustomer(customer) {
  ...
  if (!validateGeneralInformation(customer)) isValidated = false;
...
function sortListPerson() {
  ...
  switch (typePerson) {
    case "1":
      {
        tempList = listPerson.filter(
          (person) => person.constructor.name === "Student"
        );
      }
      break;
    case "2":
      {
        tempList = listPerson.filter(
          (person) => person.constructor.name === "Employee"
        );
      }
      break;
    case "3":
      {
        tempList = listPerson.filter(
          (person) => person.constructor.name === "Customer"
        );
      }
      break;
    default: {
